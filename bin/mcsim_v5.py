...
#    message.udpmessage({"_textarea":"z = [%f,%f,%f,%f]\n" % (z[0],z[1],z[2],z[3]) })
    ...
    for i in range(Number_of_models):
        sigma_poly = 0.2*a[i]
        a_range = a[i] + np.linspace(-3,3,7)*sigma_poly
        for a_j in a_range:
            if model[i] != 'none': 
            #message.udpmessage({"_textarea":"i,x[i],y[i],z[i],model[i],a[i],b[i],c[i],p[i],Npoints = %d,%f,%f,%f,%s,%f,%f,%f,%f,%d\n" % (i,x[i],y[i],z[i],model[i],a[i],b[i],c[i],p[i],Npoints) })
                res = (a_j-a[i])/sigma_poly 
                w = np.exp(-res**2/2) # give weight according to normal distribution
                vol_rel = a_j**3/a[i]**3
                Npoints_w = Npoints*(w*vol_rel)**2
                #x_new,y_new,z_new,p_new,N,rho = genpoints(x_new,y_new,z_new,p_new,x[i],y[i],z[i],model[i],a[i]*factor_d,b[i]*factor_d,c[i]*factor_d,p[i],Npoints*w*vol)
                x_new,y_new,z_new,p_new,N,rho = genpoints(x_new,y_new,z_new,p_new,x[i],y[i],z[i],model[i],a_j,b[i],c[i],p[i],Npoints_w)
                srho = rho*p[i]
    ...
    ## remmove non-zero elements (tr for truncate)
