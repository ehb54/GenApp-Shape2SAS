...
#    message.udpmessage({"_textarea":"z = [%f,%f,%f,%f]\n" % (z[0],z[1],z[2],z[3]) })
    ...
    ## adjust number of points,so total is no more than Npoints_max (due to memory issues)
    Npoints_max = 4000
    ...
    for i in range(Number_of_models):
        if model[i] != 'none': 
            ...
            ## generate points
            x_add,y_add,z_add,p_add,N,rho = genpoints(x_new,y_new,z_new,p_new,x[i],y[i],z[i],model[i],a[i],b[i],c[i],p[i],Npoints)
