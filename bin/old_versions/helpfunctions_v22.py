...
def get_volume(subunit,a,b,c):
    """ 
    calculates volume for a given subunit
    """
    Volume = 0.0

    if subunit in ['sphere','hollow_sphere']:
        SHELL=False
        if subunit == 'hollow_sphere':
            if b > a:
                R,r = b,a
            elif b == a:
                SHELL=True
    ...
    if subunit == 'ellipsoid':
        Volume = 4*np.pi*a*b*c/3
        
    if subunit in ['cylinder','disc']:
        l = c
        Volume = np.pi*a*b*l

    if subunit in ['cube','hollow_cube']:
        SHELL = False
        if subunit == 'hollow_cube':
            if b > a:
                a,b = b,a
            elif b == a:
                SHELL = True
    ...
    if subunit in ['cyl_ring','disc_ring']:
        SHELL = False
        if b > a:
            R,r,l = b,a,c
        elif b == a:
...
def gen_points(x_com,y_com,z_com,subunit,a,b,c,p,Npoints):
    """ 
    generate random uniformly distributed points (x,y,z) in user-defined shape
    subunit defines the shape in which the points should be placed
    
    input:
    a,b,c             : geometrical subunit parameters
...
def gen_all_points(Number_of_subunits,Npoints,x_com,y_com,z_com,subunit,a,b,c,p,exclude_overlap):
    """
    generate points from a collection of subunits
    calling gen_points() for each subunit

    input:
    Npoints           : max number of points per model
    x_com,y_com,z_com : center of mass coordinates
    a,b,c             : subunit params (see GUI)
    p                 : contrast for each subunit
    exclude_overlap   : if True, points is removed from overlapping regions (True/False)

    output:
    N                 : number of points in each subunit (before optional exclusion, see N_exclude)
    rho               : density of points for each subunit
    N_exclude         : number of points excluded from each subunit (due to overlapping regions)
    ...
    for i in range(Number_of_subunits):
        v = get_volume(subunit[i],a[i],b[i],c[i])
        volume.append(v)
        sum_vol += v
    ...
    x_new,y_new,z_new,p_new = x_new.astype('float32'),y_new.astype('float32'),z_new.astype('float32'),p_new.astype('float16')
...
def check_overlap(x,y,z,p,x_com,y_com,z_com,subunit,a,b,c):
    """
    ...
    ## find indices of non-excluded points, depending on subunit
    if subunit in ['sphere','hollow_sphere']:
        SHELL=False
        if subunit == 'hollow_sphere':
            if b > a:
...
def simulate_data(q,I,I0,exposure,Model):
    ...
    Model    : is it Model 1 or 2 or 3 or 4(see the GUI)
...
def plot_2D(x_list,y_list,z_list,p_list,colors,Models,high_res):
    """
...
def plot_results(q,r_list,pr_list,I_list,Isim_list,sigma_list,S_list,names,colors,colors2,scales,xscale_log,high_res):
    """
    ...
    zo = 1
    for (r,pr,I,Isim,sigma,S,model_name,col,col_sim,scale) in zip (r_list,pr_list,I_list,Isim_list,sigma_list,S_list,names,colors,colors2,scales):
        ax[0].plot(r,pr,color=col,zorder=zo,label='p(r), %s' % model_name)

        if scale > 1: 
            ax[2].errorbar(q,Isim*scale,yerr=sigma*scale,linestyle='none',marker='.',color=col_sim,label=r'$I_\mathrm{sim}(q)$, %s, scaled by %d' % (model_name,scale),zorder=1/zo)
        else:
            ax[2].errorbar(q,Isim*scale,yerr=sigma*scale,linestyle='none',marker='.',color=col_sim,label=r'$I_\mathrm{sim}(q)$, %s' % model_name,zorder=zo)

        if S[0] != 1.0 or S[-1] != 1.0:
            ax[1].plot(q,S,linestyle='--',color=col,label=r'$S(q)$, %s' % model_name,zorder=0)
            ax[1].plot(q,I,color=col,zorder=zo,label=r'$I(q)=P(q)S(q)$, %s' % model_name)
            ax[1].set_ylabel(r'$I(q)=P(q)S(q)$')
        else:
            ax[1].plot(q,I,color=col,zorder=zo,label=r'$P(q)=I(q)/I(0)$, %s' % model_name)
...
def generate_pdb(x_list,y_list,z_list,p_list,Model_list):
    """
